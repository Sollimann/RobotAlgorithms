...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        alpha = (r - l) / w
        R = l / alpha
        cx, cy = np.array([x, y]) - scanner_displacement * np.array([np.cos(theta), np.sin(theta)]) - \
                 (R + w / 2.0) * np.array([np.sin(theta), -np.cos(theta)])

        theta = (theta + alpha) % (2 * np.pi)

        x, y = np.array([cx, cy]) + scanner_displacement * np.array([np.cos(theta), np.sin(theta)]) + \
               (R + w / 2.0) * np.array([np.sin(theta), -np.cos(theta)])
        return x, y, theta
...
if __name__ == '__main__':
    ...
    pose = (1850.0, 1897.0, 213.0 / 180.0 * np.pi)
