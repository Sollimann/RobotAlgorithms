...
from ..rrt.heuristics import segment_cost, path_cost, cost_to_go

"""
Based on paper: "Incremental Sampling-based Algorithms for Optimal Motion Planning"

the RRT∗ algorithm essentially “rewires” the tree structure as it discovers
new lower-cost paths reaching the nodes that are already in the tree.
"""
...
class RRTStar(RRTBase):
    def __init__(self, X, Q, x_init, x_goal, max_samples, r, prc=0.01, rewire_count=None):
        ...
        self.c_best = float('inf')  # length of the best solution thus far
    ...
    def connect_shortest_valid(self, tree: int, x_new: tuple, L_near: List):
        """
        Connect to nearest vertex that has an unobstructed path
        :param tree: int, r-tree being added to
        :param x_new: tuple, vertex being added
        :param L_near: list of nearby vertices
        """
        # check nearby vertices for total cost and connect shortest valid edge
        for c_near, x_near in L_near:
            if c_near + cost_to_go(x_near, self.x_goal) < self.c_best and \
                    self.connect_to_point(tree, x_near, x_new):
                break

    def get_nearby_vertices(self, tree: int, x_init: tuple, x_new: tuple) -> List:
        """
        Get nearby vertices to new vertex and their associated path costs from the root of
        the tree as if new vertex is connected to each one separately

        :param tree: r-tree in which to search
    ...
    def current_rewire_count(self, tree: int) -> int:
        """
        Return rewire count
        :param tree: int, id of r-tree being rewired
        :return: rewire count
        """
        # if no rewire count specified, set require count to be all vertices
